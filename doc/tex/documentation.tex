...
\Cref{fig:gen:arch} shows the integration of the Virtex6 board and the ISE 14.1 workflow in this structure. The introduction of a new workflow like the ISE 14 requires introduction of a new corresponding board interface, providing all information required by the workflow. As an additional layer of abstraction, the general ISE backend and board interface are provided, but no direct instance of these exist. The Virtex6 board backend implements the provided ISE 14.1 board interface indicating support of this workflow. A similar pattern is applied to GPIO components. An ise gpio component interface is provided by the workflow, which is in turn implemented by the Virtex6 components. Since so far no differences in the  implementation of GPIO components for the supported ISE versions are required, a more tailored GPIO interface for each minor ISE version is not necessary. Instead, a single interface for all ISE versions is provided. If a later extension of Loopy for more versions makes such an interface necessary, a new interface can easily be introduced.
