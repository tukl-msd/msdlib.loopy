...
The following messages are sent and received by the automaton:
\begin{itemize} \itemsep1pt \parskip0pt \parsep0pt
...
\item \textbf{poll} A request for additional data.
\item \textbf{notify} Notifies the application of sending of all queued up messages.
\end{itemize}

In addition, several functions on parts of the state are introduced, to simplify the automaton.
\begin{itemize} \itemsep1pt \parskip0pt \parsep0pt
\item \textbf{size()} The number of values currently held by \texttt{q}
\item \textbf{empty()} \texttt{true}, if \texttt{size() == 0}, \texttt{false} otherwise
\item \textbf{drop(i)} Drops the first \texttt{i} values of \texttt{q}.
\item \textbf{peek(i)} Obtain the first \texttt{i} values of \texttt{q}. Returns a smaller number of values, if only less are available, i.e., \texttt{size()} \textless ~\texttt{i}.
\end{itemize}
...
\node[state, below=12em] (wait) at (init) {Wait};
\node[state, left=12em] (idle) at (wait) {Idle};
\node[state, right=12em] (ack) at (wait) {Ack};

\draw[extend, black] (anchor) to node[auto] {} (init);
%\draw[arr, black] (init) to node[midway, left=0.5em] {$\inferrule{wrt(v)}{snd(v)}$} (wait);
\draw[arr, black] (init) to node[near start, left=0.2em] {$\inferrule{[!q.empty()]}{snd(peek(n))}$} (wait);
\draw[arr, black, bend left = 20] (wait) to node[midway, above= 0.2em] {$\inferrule{ack(i)~[i==s]}{drop(i)}$} (ack);
\draw[arr, black, bend left = 20] (ack) to node[midway, below= 0.2em] {$\inferrule{[!q.empty()]}{snd(peek(n))}$} (wait);
\draw[arr, black, bend right = 40] (ack) to node[midway, right=1em] {$\inferrule{[q.empty()]}{notifiy}$} (init);
\draw[arr, black, bend right = 20] (wait) to node[midway, above=0.2em] {$\inferrule{ack(i)~[i<s]}{drop(i)}$} (idle);
\draw[arr, black, bend right = 20] (idle) to node[midway, below=0.2em] {$\inferrule{poll}{snd(peek(n))}$} (wait);
...
The control flow graphs in \Cref{sec:cfg} specify what types of messages are required. A short overview of the messages together with their type encoding and their fields is provided in \Cref{tab:protoMessages}. The following paragraphs provide a more detailed description of the message and their meaning.
