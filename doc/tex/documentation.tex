...
\draw[arr, black, bend left = 20] (ack) to node[near start, below=0.5em] {$\inferrule{!empty()}{snd(first(n))}$} (wait);
\draw[arr, black, bend right = 40] (ack) to node[midway, right=1em] {$\inferrule{empty()}{notifiy}$} (init);
\draw[arr, black, bend left = 20] (wait) to node[near end, right=0.5em] {$\inferrule{ack(i)~[i<s]}{ }$} (idle);
\draw[arr, black, bend left = 20] (idle) to node[midway, left=0.5em] {$\inferrule{poll}{snd(first(n))}$} (wait);
