...
\subsection{Katja}
The driver generator uses the Katja tool, developed by the Software Technology Group of the University of Kaiserslautern. This tool generates models for frontend and backend of the Loopy generator\footnote{Since these models will be described using their Katja specifications, it is strongly recommended to read through the Katja specification provided in form of three technical reports at \url{https://softech.informatik.uni-kl.de/Homepage/Katja}}. To be more specific, it provides the AST build up by the CUP parser as well as several models used by the generation backends. The models are described in detail in \Cref{sec:gen:models}.
...
\draw[implement, black] (iseboard14_1) to node[auto] {} (iseboard);
\draw[implement, black] (virtex6) to node[auto] {} (iseboard14_1);

\draw[extend, black] (ise) to node[auto] {} (workflow);
\draw[implement, black] (ise14_1) to node[auto] {} (ise);
...
\Cref{fig:gen:arch} shows the integration of the Virtex6 board and the ISE 14.1 workflow in this structure. The introduction of a new workflow like the ISE 14 requires introduction of a new corresponding board interface, providing all information required by the workflow. Since ISE workflows do not differ too much in minor versions, a stronger distinction between these is not required. Otherwise, an individual interface for the workflow would be required, i.e. an ISE 14.1 board interface.  The Virtex6 board backend can now implement the provided board interface indicating support of this workflow. A similar pattern is applied to GPIO components. An ise gpio component interface is provided by the workflow, which is in turn implemented by the Virtex6 components.

All provided interfaces and enums and are summed up in \Cref{tab:gen:hook}.
