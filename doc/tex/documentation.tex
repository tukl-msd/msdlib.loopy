...
\item \textbf{board backends} model the target boards of the driver generator. They are different from the other generation backends, since they do not generate code itself. Rather, they provide board-specific attributes to the actually generating backends.
\end{itemize}
...
\node[class, fill=white, below = 6em] (virtex6) at (iseboard) {Virtex6};

\node[class, fill=white, left = 6em] (workflow) at (board) {Workflow};
\node[class, fill=white, left = 6em] (ise) at (iseboard) {ISE};
\node[class, fill=white, left = 6em] (ise14_1) at (virtex6) {ISE 14.1};
...
\draw[implement, black] (virtex6) to node[auto] {} (iseboard);
...
\draw[arr] (virtex6) to node[auto] {} (ise14_1);
