...
Communication between programming logic and processing system on the Zynq platform is performed through up to two general purpose AXI slave and master ports and four high performance slave ports. The board-side driver on such a platform uses a CDMA connected to a single AXI master and one high performance slave to feed connected components. \textcolor{red}{We are considering to use this architecture on the Virtex board as well to remove the current restriction of ports.}
