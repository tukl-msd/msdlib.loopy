...
\draw[arr, black, out=240, in=300, looseness=6] (listen) to node[near end, right=1em] {$\inferrule{[full(q) \vee empty(r)]}{data(take(q, n))}$} (listen);
\end{tikzpicture}
...
This version reserves the next 4 bit of the header for the type of the message. In general, a message looks like depicted in \Cref{fig:proto:bitorder}. Values in this protocol version are expected to be properly aligned with 32-bit, meaning that, as described in \Cref{sec:arch:bitwidth}, values are padded host-side to a multiple of 32-bit and then split into 32-bit blocks.\\
