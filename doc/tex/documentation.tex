...
The HOPP Driver Generator can be called using the command line interface (CLI). While the tool generates C/C++ code, it is written in Java and therefore requires an installation of Java 6 or above. The CLI offers several parameters to futher configure the run of the generator, listed in \Cref{tab:cliParams}.
...
\node[class, below=9em] (uart) at (interface) {USB/UART};
\node[class, left=4em] (ethernet) at (uart) {Ethernet};
\node[class, right=4em] (pcie) at (uart) {PCIE};

\node[above=2em] (anchor_ethernet) at (ethernet) {};
\node[above=2em] (anchor_uart) at (uart) {};
\node[above=2em] (anchor_pcie) at (pcie) {};

\node[class, below left=4em and 0.5em] (in) at (port) {IN};
\node[class, below right=4em and 0.5em] (out) at (port) {OUT};
\node[class, below=9em] (dual) at (port) {DUAL};
...
\paragraph{Ethernet}
Communication over Ethernet is based on the lightweight IP stack, originally developed by Adam Dunkels\footnote{For documentation, a wiki exists for the lwip stack at \url{http://lwip.wikia.com/wiki/LwIP_Wiki}}.
...
\textcolor{red}{Explain the transformation of the source file into the c model.}
