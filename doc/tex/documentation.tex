...
\draw[arr, black, bend left = 20] (wait) to node[midway, above] {$\inferrule{ack(i)~[i==s]}{drop(i)}$} (ack);
\draw[arr, black, bend left = 20] (ack) to node[near start, below=0.5em] {$\inferrule{[!q.empty()]}{snd(peek(n))}$} (wait);
\draw[arr, black, bend right = 40] (ack) to node[midway, right=1em] {$\inferrule{[q.empty()]}{notifiy}$} (init);
\draw[arr, black, bend left = 20] (wait) to node[near end, right=0.5em] {$\inferrule{ack(i)~[i<s]}{drop(i)}$} (idle);
\draw[arr, black, bend left = 20] (idle) to node[midway, left=0.5em] {$\inferrule{poll}{snd(peek(n))}$} (wait);
...
%\hline
%Nibble & 0 & 1 & 2 & 3 & 4 & 5 & 6 & 7 \\ \hline
%Name & Version & & Type & ID & Size & & & \\ \hline \hline
%Value & 0000 & 0001 & 0001 & pid & size & & & \\
%\hline
%\end{tabular}
%\caption{Default data message }
%\label{tab:cliParams}
%\end{table}
