package de.hopp.generator.backends.server.zed.ise.sdk;

import static de.hopp.generator.backends.server.zed.ise.ISE.sdkSourceDir;
...
import de.hopp.generator.backends.server.zed.ise.gpio.GpioEnum;
...
//        init = addLines(init, gpio.getSDKInit());
        ...
        if(gpio.isGPI() && gpio.isGPO()) {
            errors.addError(new ParserError("Bi-directional GPIO components not allowed for Zed boards",
    ...
    private String hwVersion(GPIO gpio) throws ParserError {
        ...
        throw new ParserError("Unknown GPIO device " + gpio.name() + " for Zed", gpio.pos());
    }
...
//        boolean up = ((width < 32) && !d) || ((width > 32) && d);
    ...
    private void addWriteStream(CPUAxisPos axis) {
        if(axiStreamIdMaster>15) {
            errors.addError(new ParserError("too many writing AXI stream interfaces for Zed", "", -1));
            return;
        }
    ...
    private void addReadStream(CPUAxisPos axis) {
        if(axiStreamIdSlave>15) {
            errors.addError(new ParserError("too many reading AXI stream interfaces for Zed", "", -1));
            return;
        }
